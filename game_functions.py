...
ship_hit_sound = pygame.mixer.Sound("ship_explode.wav")
shooting_sound = pygame.mixer.Sound("shooting_sound.wav")
alien_hit_sound = pygame.mixer.Sound("alien_killed.wav")
ufo_flying_sound = pygame.mixer.Sound("Ufo_flying.wav")
...
def update_screen(ai_settings, screen, stats, sb, ship, aliens, bullets, play_button, bunkers):
...
# Becareful of alien type here
def create_alien(ai_settings, screen, aliens, alien_number, row_number):
    """Create an alien and place it in the row."""
    # make an if statement here to change the alien image if % row_number
    ...
    if int(row_number) == 3:
        alien = Alien(ai_settings, screen, 1)
    if int(row_number) == 4:
        alien = Alien(ai_settings, screen, 4)
    if int(row_number) == 5:
        alien = Alien(ai_settings, screen, 4)

    alien_width = alien.rect.width - 25  # width in between the collumns
    alien.x = alien_width + 2 * alien_width * alien_number + 300  # pushes x axis
    alien.rect.x = alien.x
    alien.rect.y = alien.rect.height + 2*alien.rect.height * row_number/2  # Space between rows
    aliens.add(alien)
...
def create_bunkers(ai_settings, screen, bunkers):
    ...
    for collumn_of_bunkers in range(number_bunkers_x):
        create_bunker(ai_settings, screen, bunkers, collumn_of_bunkers)
